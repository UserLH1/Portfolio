...
export const PinContainer: React.FC<PinContainerProps> = ({
  ...
  return (
    <Link
      className={cn(
        "relative z-40 cursor-pointer rounded-lg shadow-lg group/pin !p-0",
        containerClassName
      )}
      target="_blank"
      ...
      <div className="w-full h-full -mt-7 flex-none inset-0">
        <div className="absolute top-0 inset-x-0 flex justify-center">
          ...
          >
            ...
            <span className="absolute -bottom-0 left-[1.125rem] h-px w-[calc(100%-2.25rem)] bg-gradient-to-r from-emerald-400/0 via-cyan-700 to-emerald-400/0 transition-opacity duration-500 group-hover/btn:opacity-40"></span>
          </a>
        </div>
          ...
          <>
            {Array.from({ length: 3 }).map((_, index) => (
              <motion.div
                ...
                className="absolute left-1/2 top-1/2 h-[11.25rem] w-[11.25rem] rounded-[50%] bg-cyan-500/[0.3] shadow-[0_8px_16px_rgb(0_0_0/0.4)]"
              ></motion.div>
            ))}
          </>
        </div>
